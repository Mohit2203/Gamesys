...
import com.gamesys.test.GamesysSpringBootUserRegistration.Constant.UserRegistrationConstant;
...
public class DateValidatorImpl implements ConstraintValidator<DateValueMatch, String> {	
	...
	private static String getISO8601StringForDate(String date) {
		SimpleDateFormat dateFormat = new SimpleDateFormat(UserRegistrationConstant.isovalidDaysFormat);
	...
	public boolean isValid(String value, ConstraintValidatorContext context) {
		try {
			...
			String formattedValue = getISO8601StringForDate(value);
			if (!value.isEmpty() && value.equalsIgnoreCase(formattedValue)) {
				return true;
			} else {
				throw new DateFormatNotValidException("Please enter the date in yyyy-MM-dd format");
			}
